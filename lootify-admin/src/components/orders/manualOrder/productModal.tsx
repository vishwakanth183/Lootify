...
const ProductModal: FC<{ product: productItem | null, closeFunction: any }> = ({ product, closeFunction }) => {
    ...
    const getProductDetails = async () => {
        await HttpRoutingService.getMethod("product/getProductDetailById", { productId: productProps?.id }).then((res: any) => {
    ...
    const addToCart = () => {
        ...
        if (productDetail?.isVariants) {
            ...
            newCartItem["quantity"] = 1,
                newCartItem["salesPrice"] = selectedVariant?.salesPrice;
    ...
    return <Box pt={3}>
        ...
        <Stack display={"flex"} alignItems={"flex-start"} direction={"row"}>
            ...
            <Image
                ...
                alt={productProps?.productName + "image"}
                width={150}
                height={150}
            />
            ...
            <Box pl={2}>
                <Typography variant="h6" color={"purple"}>{productDetail?.productName}</Typography>
                <Stack display={"flex"} direction={"row"} spacing={1}>
                    <Typography sx={{ textDecoration: 'line-through' }} color={"grey"}>₹{productDetail?.isVariants ? selectedVariant?.mrpPrice : productDetail?.mrpPrice}</Typography>
                                                ...
                                                return <Box key={index}>
                                                    <Stack direction={"row"} display={"flex"} alignItems={"center"}>
                                                        <IconButton onClick={() => onChangeVariant({ optionId: options.id, optionValueId: optionValue.id })}>
                                                            {optionValue.color ? checkSelectedValue(optionValue.id) ? <RadioButtonChecked htmlColor={optionValue.color} /> : <Circle htmlColor={optionValue.color} /> : checkSelectedValue(optionValue.id) ? <RadioButtonChecked htmlColor={optionValue.color} /> : <RadioButtonUncheckedOutlined />}
                                                        </IconButton>
                ...
                {!productDetail?.quantity && !selectedVariant?.quantity ? <Button variant="contained" color="success" disabled={!(Number(productDetail?.isVariants ? selectedVariant?.stock : productDetail?.stock))} onClick={() => addToCart()}>
                    Add to cart
                </Button> :
                    <Stack direction={"row"} display={"flex"} alignItems={"center"} spacing={2}>
                        <IconButton sx={{ bgcolor: "lightgrey" }} onClick={() => { updateQuantity("decrement") }}>
                            <Remove fontSize="small" />
                        </IconButton>
                        <Typography color={"black"}>{product?.isVariants ? selectedVariant?.quantity : productDetail?.quantity}</Typography>
                        <IconButton sx={{ bgcolor: "lightgrey" }} onClick={() => { updateQuantity("increment") }}>
                            <Add fontSize="small" />
                        </IconButton>
                    </Stack>
                }
